...
# *Note, the .csv file in its raw downloaded form has a title
#   value as the first record, which messes up the read.
#   open the .csv file and delete the first row and should work

# load a csv file
loader_path1 <- paste0(getwd(), "/etl/ore/events-US-1980-2022.csv")
clockin()
...
dfa <- raw_df %>%
  ...
  # parse the dates :::::::::::::::
  mutate(begin_yr = stringr::str_sub(as.character(begin_date), 
                                     start = 1L, 
                                     end = 4L), 
         begin_yr = as.integer(begin_yr), 
         begin_mon = stringr::str_sub(as.character(begin_date), 
                                      start = 5L, 
                                      end = 6L), 
         begin_yrmon = paste0(as.character(begin_yr), '-', 
                              begin_mon)) %>% 
  # fix cost of disaster ::::::::::::::
  rename(cpi_adj_costs_mill = total_cpi_adjusted_cost_millions_of_dollars) %>% 
  mutate(cpi_adj_costs_mill = as.double(cpi_adj_costs_mill))
clockout()
